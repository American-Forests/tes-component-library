...
it('fires onClick handler exactly once when clicked', () => {
    ...
    const { getByRole } = render(<Button text="Add to scenario" onClick={onClick} />)
...
it('renders the right tailwind classes for the primary variant', () => {
    const { getByRole } = render(<Button text="Add to scenario" />)
    expect(getByRole('button')).toHaveClass('text-white bg-brand-green hover:bg-brand-green-dark')
})
...
it('renders the right tailwind classes for the secondary variant', () => {
    ...
    expect(getByRole('button')).toHaveClass('text-brand-green bg-white border-brand-green border-2 hover:bg-brand-green hover:text-white')
})
